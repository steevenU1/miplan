<?php
// ajax_scooters_por_sucursal.php
// Lista SOLO scooters disponibles por sucursal para el flujo de venta de scooters
...
// Si no tenemos columna de tipo, no podemos filtrar scooters
if ($col_tipo === null) {
  echo '<option value="">No se puede filtrar scooters (falta columna tipo en productos)</option>';
  exit;
}
...
$cols[] = "LOWER(TRIM(p.`$col_tipo`)) AS tipo";

// 🎯 Filtro específico de SCOOTERS
$filtroScooter = " AND p.`$col_tipo` IS NOT NULL
                   AND LOWER(TRIM(p.`$col_tipo`)) IN ('scooter','scooters')";
 ...
 */
$res = $stmt->get_result();
$options = '<option value="">Seleccione un scooter...</option>';
...
if ($res instanceof mysqli_result) {
  ...
  if ($res->num_rows > 0) {
    ...
    $options .= '<option value="">Sin scooters disponibles en esta sucursal</option>';
  }
} else {
  // Camino sin mysqlnd
  $stmt->store_result();
...
function pintarOpcion(array $row): string {
  ...
  $imeiTxt = $imei1 !== '' ? "IMEI1/Serie: $imei1" : "IMEI1/Serie: —";
