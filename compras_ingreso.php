...
if (isset($_GET['action']) && $_GET['action'] === 'check_imei') {
  ...
  if ($imei === '' || !preg_match('/^\d{15}$/', $imei)) {
    $resp['msg'] = 'Formato inválido: 15 dígitos.';
  ...
  if ($st = $conn->prepare($sql)) {
    ...
    if ($row = $res->fetch_assoc()) {
      $resp = ['ok' => true, 'exists' => true, 'field' => $row['campo'] ?: 'desconocido', 'msg' => 'Duplicado en BD'];
      echo json_encode($resp);
      exit;
    }
    $st->close();
  }
  $resp = ['ok' => true, 'exists' => false, 'msg' => 'Disponible.'];
  echo json_encode($resp);
  exit;
}
...
if ($proveedorCompra !== '') {
  $proveedorCompra = mb_substr($proveedorCompra, 0, 120, 'UTF-8');
}
...
function sugerirPrecioLista(mysqli $conn, ?string $codigoProd, string $marca, string $modelo, string $ram, string $capacidad, float $costoConIva, ?float $precioCat)
{
  if ($codigoProd) {
    ...
    $qInv->execute();
    $qInv->bind_result($plInv);
    if ($qInv->fetch()) {
      $qInv->close();
      return ['precio' => (float)$plInv, 'fuente' => 'inventario vigente (mismo código)'];
    }
    $qInv->close();
  }
  ...
  if ($q2->fetch()) {
    $q2->close();
    return ['precio' => (float)$pl2, 'fuente' => 'último por modelo (RAM/cap)'];
  }
  $q2->close();
...
function ultimoSubtipo(mysqli $conn, ?string $codigoProd, string $marca, string $modelo, string $ram, string $capacidad)
{
  if ($codigoProd) {
    ...
    if ($q->fetch()) {
      $q->close();
      return ['subtipo' => $st, 'fuente' => 'por código'];
    }
    $q->close();
  }
  ...
  if ($q2->fetch()) {
    $q2->close();
    return ['subtipo' => $st2, 'fuente' => 'por modelo (RAM/cap)'];
  }
  $q2->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($esAccesorio) {
    ...
    try {
      ...
      if ($pid === 0) {
        ...
        if (!$insProd->execute()) {
          throw new Exception("Crear producto accesorio: " . $insProd->error);
        }
  ...
  ON DUPLICATE KEY UPDATE
    ...
    estatus = 'Disponible'
");
      $up->bind_param('iii', $pid, $enc['id_sucursal'], $cantidadMarcar);
      if (!$up->execute()) {
        throw new Exception('UPSERT inventario: ' . $up->error);
      }
      $up->close();
      ...
      if ($cantidadMarcar > 0) {
        ...
        for ($i = 0; $i < $cantidadMarcar; $i++) {
          $null = null;
          $stmtR->bind_param("issi", $detalleId, $null, $null, $pid);
          if (!$stmtR->execute()) {
            throw new Exception("Registrar ingresos: " . $stmtR->error);
          }
        }
        $stmtR->close();
      }
  ...
  <script>
    (function() {
      ...
      form.addEventListener('submit', (e) => {
        ...
        if (form.querySelector('.dup-bad')) {
          e.preventDefault();
          alert('Hay IMEI duplicados.');
          return;
        }
        form.dataset.busy = '1';
        if (btn) {
      ...
      function markDup(el, msg, bad = true) {
        const help = document.getElementById('dupmsg-' + el.id);
        if (bad) {
          el.classList.add('is-invalid', 'dup-bad');
          if (help) {
            help.classList.remove('d-none');
            help.textContent = msg || 'Duplicado.';
          }
        } else {
          el.classList.remove('dup-bad', 'is-invalid');
          if (help) {
      ...
      function checkLocal() {
        ...
        map.forEach(arr => {
          if (arr.length > 1) arr.forEach(el => markDup(el, 'Duplicado en formulario', true));
        });
      }
      const checkRemote = debounce(async (el) => {
        const v = (el.value || '').replace(/\D+/g, '');
        if (v.length !== 15 || !luhnOk(v)) return;
        try {
          ...
          if (data && data.ok) {
            if (data.exists) markDup(el, `Duplicado en BD (${data.field})`, true);
